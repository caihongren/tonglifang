  ...
  <div class="simulationResourcesOverallgroup">
    ...
    <div class="simulationResourcesOverall">
      <div class="simulationResourcesOverall-top">
        <p class="simulationResourcesOverall-left">自由创新实验</p>
        ...
        <el-table :data="tableDataFalse" stripe :default-sort="{prop:'createdAt', order: 'descending'}" :header-cell-style="{background:'#ebeffb'}" class="Simulation operation" :row-class-name="tableRowClassName">
          ...
          <el-table-column prop="createdAt" label="保存时间" min-width="90" sortable>
            <template slot-scope="scope">{{scope.row.createdAt|dateformat}}</template>
          </el-table-column>
          <el-table-column prop="typeName" label="说明" min-width="150"></el-table-column>
    ...
    <!-- 新建弹出框 -->
    <el-dialog :visible.sync="testTemplateLibrary" width="40%" title="新建自由创新实验">
      <div>
        <div style="margin:30px 0">
          <span style="width:120px;display: inline-block;">名称：</span>
          <el-input placeholder="请输入名称" v-model="addform.name" clearable style="width:80%"></el-input>
        </div>

        <div style="margin:30px 0;height: 50px;line-height: 50px;">
          <span style="width:120px;display: inline-block;">说明：</span>
          <el-input
            type="textarea"
            :autosize="{ minRows: 2,}"
            placeholder="请输入说明"
