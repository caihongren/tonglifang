    ...
    <div class="box">
      ...
      >
        ...
        <el-form-item label="姓名：" prop="name">
          <el-input v-model="form.name"  maxlength='20' auto-complete="off" placeholder="请输入名称（必填）"></el-input>
        </el-form-item>
        ...
        <el-form-item label="电话："  prop="phone" disabled>
          <el-input v-model="form.phone"  maxlength='11' auto-complete="off" placeholder="请输入电话号码（必填）"></el-input>
        </el-form-item>
      ...
      <div style="width:100%;height:50px;">
        ...
        <div style="  float: left;padding-left:100px" class="dialog-footer"  v-show="isweixing">
          <el-button type="success" size="small" class="mima"  @click="$router.push('/weixinglogin')" v-show="!weixinglogins">绑定微信</el-button>
          <el-button type="danger" size="small" class="mima"  @click="wechat_unbound()" v-show="weixinglogins"> 解绑微信</el-button>
        </div>
...
export default {
  data() {
    ...
    return {
      isweixing: false, //是否显示微信
      weixinglogins: false, //是否绑定微信
      curlist: [],
      status: "",
  ...
  methods: {
    ...
    // 是否绑定微信
    wechat_check() {
      wechat_check().then(res => {
        if (res.data.code == 0) {
          this.isweixing = true;
          this.weixinglogins = false;
        } else if (res.data.code == -1) {
          this.isweixing = true;
          this.weixinglogins = true;
        } else {
          this.isweixing = false;
        }
      });
    },
    // 微信解绑
    wechat_unbound() {
      wechat_unbound().then(res => {
        if (res.data.code == 0) {
          this.$message({
            showClose: true,
            duration: 1000,
